...
- **Per-file inspection output is ordered** — the multi-file inspection commands (nvddsinfo, and texdiag info/analyze/dumpbc/dumpdds) still read files in parallel, but each file's report is collected and printed in sorted file order at the end as one clean block, instead of worker-completion order interleaving the log.
